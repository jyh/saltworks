...
def to_lean(gates, ns, name, outs, ninputs, src, state=(), ndesign_in=None,
            ndesign_out=None, clockinfo=None, cuts=(), pinned=None, out_names=(), omitted=(),
            in_names=()):
    # ⛔ THE LICENCE HEADER IS EMITTED BY THE GENERATOR, NOT PATCHED INTO ITS OUTPUT.
    # These datums are regenerated by reimport.sh, so a hand-added header on the
    # emitted file is a fix with a timer: the next reimport silently drops it and a
    # PUBLIC repo loses the notice again. Same rule as every other thing this datum
    # must carry — if the importer must emit it, the importer emits it.
    # (Gap found 8/16 20:1x: 8 of these files were headerless in the public repo.)
    L = [f"/-",
         f"Copyright (c) 2026 Jason Hickey. All rights reserved.",
         f"Released under Apache 2.0 license as described in the file LICENSE.",
         f"Authors: Jason Hickey, Claude",
         f"-/",
         f"-- GENERATED by SaltWorks/Silicon/Importer/import_netlist.py",
    ...
    L.append("")
    # ⛔ THE OLD DOCSTRING HERE SAID "in declaration order" AND WAS FALSE — it
    # shipped in all seven live datums. Declaration order is DISCARDED at parse
    # (`decls` is a defaultdict(set)); the emitted order is the CALLER's
    # --outputs order, then Q-sorted state, then cuts. The correction rides the
    # same commit as the table below, per the helm ruling of 2026-08-13 18:05:38
    # — and it is the strongest argument FOR the table: a reader had no way to
    # check `outs[k]` against anything, and the one sentence that claimed to
    # help pointed at an order this importer never had.
    L.append(f"/-- Output net indices, in the CALLER'S `--outputs` order (then state,")
    L.append(f"then cuts). NOT declaration order — this importer discards that. -/")
    L.append(f"def {name}_outs : List Nat := {outs}")
    if out_names:
        L.append("")
        # ⭐ THE NAME TABLE — A2′ rev-3. `outs[k]` was checkable against nothing:
        # the datum recorded indices and no names, so the binding of k to a port
        # lived only in an argument on someone's command line. DESIGN outputs
        # only (the `outs` prefix); state and cut entries stay unnamed because a
        # flop D net and a cut root are not ports — their pairing is already
        # recorded by position and by the flop table's Q column.
        # ⚠️ NOT sorted(), DELIBERATELY: this list derives from an ORDERED caller
        # argument, not from a set, and the order IS the index semantics. The
        # determinism rule (sort anything set-derived, because two arms are two
        # python3 processes with per-process hash order) does not apply — sorting
        # here would destroy the very property the table exists to record.
        L.append(f"/-- DESIGN output NAMES, positionally aligned with `{name}_outs`:")
        L.append(f"`{name}_outs[k]` is the net carrying the port named at position k.")
        L.append(f"Each name is VALIDATED against the module's declared outputs (C-V3),")
        L.append(f"never derived from declaration order. One entry per EMITTED BIT. -/")
        # ⛔ DOUBLE QUOTES, AND THE BUILD IS WHAT TAUGHT ME: python's list()
        # renders 'q0' with SINGLE quotes, and in Lean `'x'` is a CHARACTER
        # literal — so `'cout'` is a malformed char, not a string, and every
        # regenerated datum failed to elaborate with "missing end of character
        # literal". ⚠️ AND NOTE WHICH CHECK CAUGHT IT: every A2′ rev-3 predicate
        # PASSED on those files, because the criterion greps the table's TEXT and
        # cannot see that the text is not valid Lean. A grep-level criterion over
        # a Lean datum is blind to the datum being Lean at all — the build row is
        # not a formality in that bar, it is the only arm that reads the language.
        _names_lit = "[" + ", ".join('"' + n + '"' for n in out_names) + "]"
        L.append(f"def {name}_out_names : List String := {_names_lit}")
    # ⭐ EMITTED UNCONDITIONALLY — math's ruling 2026-08-13 18:31:46, refusing the
    # alternative of a two-case predicate. THE ANCHOR MUST BE A PROPERTY OF THE
    # DATUM, NOT OF THE DATUM'S CLASS: if this field existed only on sequential
    # datums, the criterion `table.length == ndesign_out` would silently mean one
    # thing on a flopped datum and another on a combinational one — SAME FORMULA,
    # DIFFERENT THEOREM, with nothing in the output saying which you just proved.
    # ⚠️ And on a combinational datum the value is not INAPPLICABLE, it is
    # DEGENERATE-BUT-DEFINED: with no state to separate, the design outputs ARE
    # all the outputs, so it equals len(outs) by construction. A canonical form
    # RECORDS a defined value; it does not omit it and make the reader infer it
    # from the datum's class.
    # --- OMITTED DECLARED OUTPUTS — DECLARE, never refuse, never warn.
    # Helm ruling 18:05:38 (2). Refusal is wrong: a sub-cone import is legitimate,
    # which is the half I deliberately left untaken when C-V3 landed. A warning
    # is a CARRIED LINE, and this fleet measured today what one is worth — a
    # blocker phrase repeated 41 times without once being re-derived.
    # ⛔ PRESENT EVEN WHEN EMPTY, and that is the whole design: an ABSENT line is
    # indistinguishable from an old-format datum, while a PRESENT EMPTY line is a
    # MEASURED ZERO. The distinction between "nothing was omitted" and "nobody
    # looked" is exactly what this seat spent the day failing to make.
    # ⚠️ sorted() HERE, unlike the name table three lines up — and the contrast is
    # the rule, not an inconsistency: this derives from a SET (hash order is
    # per-process random, and the two arms are two python3 processes), while the
    # name table derives from an ORDERED caller argument whose order IS the index
    # semantics. Same determinism rule, opposite call, different objects.
    # ⛔ NAME: `_outs_omitted`, NEVER `_omitted_outs`. readback.py:321 does a
    # FIRST-MATCH `re.search(r"_outs : List Nat := ...")`, and `_omitted_outs`
    # contains that exact substring — it would hijack the readback of a datum it
    # was added to describe.
    L.append("")
    L.append(f"/-- Declared outputs the caller did NOT request, sorted. PRESENT EVEN")
    L.append(f"WHEN EMPTY: an absent line means an old-format datum, an empty line is a")
    L.append(f"measured zero. A sub-cone import is legitimate — this DECLARES the scope")
    L.append(f"rather than refusing it, so a reader can see what the datum leaves out. -/")
    L.append(f"def {name}_outs_omitted : List String := ["
             + ", ".join('"' + o + '"' for o in sorted(omitted)) + "]")
    L.append("")
    # --- DESIGN INPUT NAMES — SHAPE A, helm ruling 2026-08-15 06:49:22.
    # ⛔ EMITTED UNCONDITIONALLY, PRESENT EVEN WHEN EMPTY — the ruling names math's
    # emit-unconditionally ANCHOR doctrine, not `_out_names`'s conditional shape.
    # My dream patch guarded this with `if in_names:` and I mirrored the wrong
    # sibling: an ABSENT line is indistinguishable from an old-format datum, while
    # a PRESENT EMPTY line is a measured zero. Byte-identical on today's five (all
    # pass --inputs); the readings diverge only for a future no-names import.
    # ⚠️ Positionally aligned with the DESIGN inputs and therefore bounded by
    # `_ndesign_in`, exactly as `_out_names` is bounded by `_ndesign_out`. State Q
    # nets and cut roots are NOT named here — they are not ports.
    # Q3 (same ruling): AVAILABLE, not compelled. NAMING IMPLIES CHECKING — any
    # theorem or docstring that CITES a port name checks it against this table.
    L.append(f"/-- DESIGN input NAMES, positionally aligned with the primary input")
    L.append(f"indices: input i is the port named at position i, for every i < this")
    L.append(f"list's length. Lets a proof cite a RECORDED name rather than trusting the")
    L.append(f"caller's --inputs order. ⚠️ CROSS-CHECK AVAILABILITY IS CLASS-DEPENDENT:")
    L.append(f"`_ndesign_in` is emitted ONLY for datums carrying state, so a stateful")
    L.append(f"datum can check this list's length against it and a combinational one")
    L.append(f"cannot. That asymmetry is the very defect this file's ndesign_out comment")
    L.append(f"forbids — an anchor that is a property of the datum's CLASS — and it is")
    L.append(f"reported to the helm rather than fixed here, because the ruling asked for")
    L.append(f"NAME TABLES and changing another field's emission is scope I was not given. -/")
    L.append(f"def {name}_in_names : List String := ["
             + ", ".join('"' + n + '"' for n in in_names) + "]")
    L.append("")
    _ndo = len(outs) if ndesign_out is None else ndesign_out
    L.append("")
    L.append(f"/-- Number of DESIGN outputs. `{name}_outs` entries from here on are")
    L.append(f"next-state bits then cut roots, paired by position with the state inputs;")
    L.append(f"on a datum with neither, this is simply the length of `{name}_outs`. -/")
    L.append(f"def {name}_ndesign_out : Nat := {_ndo}")
    if state:
        n = len(state)
        L.append("")
...
# The OMITTED set: bit-expansion of vector declared outputs UNION scalar declared
# outputs, MINUS what the caller requested. ⛔ The naive "declared minus
# requested" is unsatisfiable and non-discriminating on exactly the vector ports
# that motivate this — `decls` holds BASE names while C-V1 forces bit callers, so
# `rdata` would read as "omitted" while all 32 of its bits were requested. That is
# the same trap that made C-V3's first draft mutually unsatisfiable with C-V1.
_declared_bits = set()
for _o in _declared_outs:
    if _o in vports:
        _hi, _lo = vports[_o]
        _declared_bits.update(f"{_o}[{k}]" for k in range(min(_hi, _lo), max(_hi, _lo) + 1))
    else:
        _declared_bits.add(_o)
_omitted_outs = sorted(_declared_bits - set(outs_named))
...
open(a.out, "w").write(to_lean(gates, a.ns, a.name, out_idx, len(ins_all), a.netlist,
                               ...
                               out_names=outs_named, in_names=ins,
                               omitted=_omitted_outs))
