        ...
        if (!rst_n) begin in_acc <= 32'd0; instr_r <= 32'd0; rdata_r <= 32'd0; end
            ...
            case (phase)
                ...
                2'd3: begin
                    ...
                    // ⚠️ HYPOTHETICAL — EXECUTOR SCRATCH COPY, NOT A PROPOSAL AND NOT LANDED.
                    // The committed line is `if (kind == T_FETCH) instr_r <= ...`,
                    // ungated. That lets the fetch loop that DISCOVERS a stall
                    // overwrite the stalling instruction before its memory loops run,
                    // so the store's address/data come from the NEXT instruction.
                    // The obvious repair is to commit the fetch only on retire.
                    // This file exists to measure what that repair does.
                    if (kind == T_FETCH && retire) instr_r <= {pin_in, in_acc[23:0]};
