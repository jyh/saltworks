...
// ============================================================================
module tb;
  parameter integer SOF_AT_RETIRE = 0;  // 1 = pulse sof on a retiring phase-3 edge
...
`endif
  ...
  // ── INJECTED PROBE (silicon 09/03): DOES `sof` AT A RETIRING PHASE-3 EDGE
  //    RE-ISSUE THE COMPLETED TRANSACTION?
  // The `sof` arm of busadapt8's arbitration re-derives `kind` from `c_dmem_req`
  // and does NOT consult `retire`, while the `loop_end` arm does (ratified shape
  // A, 08/18). `c_dmem_req` is a pure decode of the instruction still in
  // `instr_r`, so it cannot fall until a new instruction is fetched. Prediction:
  // a one-cycle `sof` here re-issues the transaction instead of advancing.
  // ONE VARIABLE. The host is the tracked combinational one, unchanged.
  integer sof_pulses = 0;
  integer n_retires = 0, n_loadloops = 0, n_storeloops = 0;
  always @(posedge clk) if (rst_n) begin
    if (retire_w) n_retires = n_retires + 1;
    if (dut.u_bus.phase==2'd0 && dut.u_bus.kind==2'b10) n_loadloops  = n_loadloops + 1;
    if (dut.u_bus.phase==2'd0 && dut.u_bus.kind==2'b11) n_storeloops = n_storeloops + 1;
  end
  always @(posedge clk) if (rst_n && SOF_AT_RETIRE != 0) begin
    // ARM 1: a RETIRING phase-3 edge (the store's data loop completing).
    if (SOF_AT_RETIRE == 1 && dut.u_bus.phase == 2'd3 && retire_w && sof_pulses == 0
      ...
      $display("    >> PROBE arm 1: sof at a RETIRING phase-3 edge (store data loop)");
    end
    // ARM 2 — THE FAIRNESS CONTROL. A NON-retiring phase-3 edge, i.e. mid
    // transaction: this is what `sof` is FOR (the 08/18 repair made it reframe
    // the transaction cleanly). If arm 2 is ALSO damaging, the finding is "sof is
    // broken"; if arm 2 is CLEAN, the finding is the narrow and correct one —
    // the `sof` arm does not consult `retire`, so only a RETIRING edge re-issues.
    else if (SOF_AT_RETIRE == 2 && dut.u_bus.phase == 2'd3 && !retire_w && sof_pulses == 0) begin
      sof <= 1'b1; sof_pulses = sof_pulses + 1;
      $display("    >> PROBE arm 2: sof at a NON-retiring phase-3 edge (mid transaction)");
    end
    else sof <= 1'b0;
  end
  ...
  initial begin
    ...
    $display("  MEASURED_FINAL: x1=%h x3=%h retires=%0d load_loops=%0d store_loops=%0d sof_pulses=%0d",
           dut.u_core.regs[1], dut.u_core.regs[3], n_retires, n_loadloops, n_storeloops, sof_pulses);
    $finish;
  end
endmodule
