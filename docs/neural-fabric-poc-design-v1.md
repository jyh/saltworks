...
| cell v accumulates b + Σ (W·x) with the sign cycle correct | `MacInduction` (ℤ arithmetic, 12 rows, mutants run) + `MacBridge` rungs 1–3 (hardware: the cell ADDS its trace, BitVec) | **LANDED, JOINED (8/9 15:46): the composing theorem exists (c732aaa, first attempt) — trace + sign cycle + mac_correct = b + W·sval, hypotheses stated (hW · prefix no-overflow · ¬ssubOverflow, all discharged at demo scale). The cell computes the signed dot product, at the kernel model; down-to-silicon closes at emitS + synthesis (F3).** What is verified in hardware is that the cell adds what it is fed; that the fed sequence *means* b + W·x closes at rung 4, the rung carrying the chain's only hypothesis (math's exact text, 14:28) |
