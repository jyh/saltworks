...
| ⛔ **`#audit_axioms` ITSELF — and this is the campaign's headline instrument** | does this theorem's proof rest on only three axioms? | **does this theorem EXIST?** ⛔ **It printed a cheerful `✓ [0 axioms]` for two theorems that had NOT ELABORATED** (heartbeat timeout). **The audit is blind to elaboration failure** — only `saltbuild EXIT=1` caught it. ***A green ✓ from `#audit_axioms` is not evidence the theorem exists.*** *(silicon seat, 14:04 — the third instance today of "read the text, not the number", now applied to the audit line itself.)* |
...
| 4a | ⭐ **THE CONSTRUCTIVE HALF OF "THE FLOW FLATTENS", and it belongs beside that finding rather than under it (`002abc1`, silicon, at their request).** **FLATTENING IS A FLOW SETTING, NOT A LAW.** Marking the stage boundaries `(* keep *)` makes them **survive synthesis as real nets** — `wire [7:0] w0; wire [7:0] w1;` appear in the netlist — at a **measured 1.7% area cost** (2,108 → 2,143 µm²). With those as cut points, **per-cone coverage rises 86.9% → 94.8%.**<br><br>**So: if you want the boundaries you reason about to survive to the fabricated netlist, ASK THE FLOW TO KEEP THEM.** Two percent of area is the difference between a netlist whose structure matches the design's and one optimised into a single 36-input cone. *The tools flatten by default; they do not flatten by necessity.*<br><br>⚠️ **NOT YET ESTABLISHED, and now on the D5 list: whether `(* keep *)` survives TT's OWN CI flow** — which is the only flow whose output we will actually prove things about. | silicon to verify at D5 |
