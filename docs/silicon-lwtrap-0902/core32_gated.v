// core32_gated.v — core32.v with EXACTLY one change: rd is not written on a trapping
// word load. Generated from SaltWorks/Silicon/RTL/core32.v by docs/silicon-lwtrap-0902/run.sh
// (a diff against the source is printed by the runner; it must be the reg_we block only).
...
module core32_gated(clk, rst_n, en, instr, dmem_rdata, dmem_addr, dmem_wdata, dmem_be,
    ...
    // ⛔ THE TRAP GATE (R9a's one wire, the RTL side): a word load whose address the
    // kernel classifies misaligned or outOfRange (HDL/ISA.lean:111 addrClass —
    // outOfRange ↔ byte address ≥ 32, tested FIRST; else misaligned ↔ addr % 4 ≠ 0)
    // must NOT write rd. ONLY the register write is gated; dmem_req / dmem_we are
    // untouched so DriveMap (req = isLW ∨ isSW, we = isSW) stays true by construction.
    wire ld_trap = is_load_w & ((alu_y >= 32'd32) | (alu_y[1:0] != 2'b00));
    wire reg_we = is_lui|is_auipc|is_jal|is_jalr|(is_load_w & ~ld_trap)|is_immop|is_regop;
