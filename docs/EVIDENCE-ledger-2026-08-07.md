...
Generated 2026-08-07 19:14 America/Los_Angeles by `docs/ledger-tools/silence_windows.py` (saltworks, EVIDENCE seat).
...
A commit is made **by** a session, and a session writes records. So a commit landing where no personal-lane session wrote anything at all is **proof of a hole in the transcript record**, not evidence that nobody was directing. Checked against 374,089 liveness records at a **5-minute** tolerance.
...
⛔ **1 of 430 commits landed in a stretch with NO transcript record.** Every silence figure below that contains one of these is a **lower bound on presence**: the human may have been directing and the evidence is missing, not absent. Do not publish a window containing these commits as unattended.
...
|---|---:|
| Commits | **430** |
| `.lean` lines inserted | **24,098** |
| All lines inserted | 235,132 |
| First commit | 2026-08-05 22:05 `4fa92be` |
| Last commit | 2026-08-07 19:12 `500aa93` |
| Human touches read, personal-lane fleet (whole transcript record) | **2,406** |
| — of which into this seat (`-Users-jyh-projects-claude-saltworks`) | 221 |
...
Inside the commit window itself, the fleet received **518** human touches: **218** into this seat and **300** into every other personal-lane seat combined.
...
| ≥ 8 h | 0 | 0.0% | 0 |
| ≥ 12 h | 0 | 0.0% | 0 |
| (observed by this seat) | 425 | 100% | 23,835 |
...
| **21 min** | 2026-08-07 15:13 | 2026-08-07 15:34 | 5 | 52 |
| **21 min** | 2026-08-07 16:12 | 2026-08-07 16:33 | 8 | 434 |
| **18 min** | 2026-08-06 16:15 | 2026-08-06 16:34 | 3 | 0 |
| **18 min** | 2026-08-07 11:20 | 2026-08-07 11:39 | 5 | 256 |

**Best exhibit by commits landed:** 0h 29m of silence (2026-08-07 15:34 → 2026-08-07 16:03) carrying **14 commits** and **1,489 `.lean` lines**.

## 5. The longest unbroken run

**14 consecutive commits**, 2026-08-07 15:40 → 2026-08-07 16:00, span **0h 19m**, with zero human touches to any personal-lane seat between the first and the last.

| Time | Commit | Subject |
|---|---|---|
| 08-07 15:40 | `483fa39` | saltworks: the reach census — a Lean-side instrument for "what is this theorem ABOUT", … |
| 08-07 15:41 | `29d7078` | saltworks: P1 CONCENTRATION IS PROVED — the actives are exactly Iio n, for all 256 acti… |
| 08-07 15:44 | `d855e9b` | saltworks: obligation (a) — the key the convention-C element orders by |
| 08-07 15:45 | `c21359f` | saltworks: PARTIAL LOAD CLOSES — the switch routes its actual operating range, with hyp… |
| 08-07 15:46 | `d52a9bb` | saltworks: C4DECOMP -- the key order owned in math's lane, and C4Spec decomposed fieldwise |
| 08-07 15:47 | `99f170b` | saltworks: C4DECOMP addendum -- the count is 61 not 60, and silicon's concurrently-land… |
| 08-07 15:50 | `1916c44` | saltworks: the key-order collision resolved by BRIDGING, not by a third order |
| 08-07 15:50 | `afb5e1f` | saltworks: partial load's controls — it BITES at bnCSparse's own pattern, five idle lin… |
| 08-07 15:53 | `d103617` | saltworks: the PROVEN-vs-BUILT census — the literal class is EMPTY, the behavioural cla… |
| 08-07 15:54 | `ca6c368` | saltworks: obligation (b) at the element — the hardware realises the key, and my SPEC w… |
| 08-07 15:54 | `c1b2681` | saltworks: the address bound VANISHES at the fabricated width — taking math's Dest 3 br… |
| 08-07 15:56 | `b0cff25` | saltworks: link ② goes through the Dest 3 door — measured, not preferred |
| 08-07 15:59 | `add505a` | saltworks: import-closure counts the LIBRARY, not every tracked .lean — my own inflated… |
| 08-07 16:00 | `a6d0261` | saltworks: the census tool declares itself a non-library metaprogram |
...
| 2026-08-07 | Fri | 233 | 17,173 | 0 | 224 | 01:45 | 19:14 |

## 7. The night column — reported so it is never quoted

Commits in 21:00–04:59 local: **19 of 430 (4.4%)**.
...
|---|---|---:|
| `typed` | **counted as human** | 2,138 |
| `slash-command` | **counted as human** | 157 |
| `legacy-fallback` | **counted as human** | 85 |
| `interrupt` | **counted as human** | 26 |
| `tool-result` | rejected | 16,131 |
| `task-notification` | rejected | 2,204 |
| `slash-command-echo` | rejected | 310 |
| `harness-injection` | rejected | 206 |
| `compaction-summary` | rejected | 31 |
...
**Queue correction.** 860 messages were typed while the model was busy; the transcript writes them at dequeue time. Their `queue-operation: enqueue` timestamps were used instead. Largest correction applied: 464 s.

**Parse totals.** 120,993 records over 22 session files; 21,293 carried `type: user`; 0 lines failed to parse.
...
A commit is made **by** a session, and a session writes records. So a commit landing where no personal-lane session wrote anything at all is **proof of a hole in the transcript record**, not evidence that nobody was directing. Checked against 374,095 liveness records at a **5-minute** tolerance.
...
| Human touches read, personal-lane fleet (whole transcript record) | **2,406** |
| — of which into this seat (`-Users-jyh-projects-claude-salt`) | 2,144 |
| Seats read for presence | 5 |
| Transcripts observe from | 2026-07-07 07:18 |
...
| 2026-08-07 | Fri | 3 | 272 | 0 | 224 | 01:45 | 19:14 |
...
**Parse totals.** 120,997 records over 22 session files; 21,293 carried `type: user`; 0 lines failed to parse.
...
A commit is made **by** a session, and a session writes records. So a commit landing where no personal-lane session wrote anything at all is **proof of a hole in the transcript record**, not evidence that nobody was directing. Checked against 374,107 liveness records at a **5-minute** tolerance.
...
**Parse totals.** 121,003 records over 22 session files; 21,294 carried `type: user`; 0 lines failed to parse.
...
Generated 2026-08-07 19:14 America/Los_Angeles by `docs/ledger-tools/token_meter.py` (saltworks, EVIDENCE seat), per `docs/measurement-preregistration.md` §1.
...
| Quantity | Tokens |
|---|---:|
| API requests (deduplicated) | 13,920 |
| Input | 104,549 |
| **Output** | **9,047,317** |
| Cache created | 61,981,847 |
| Cache read | 4,168,838,703 |
| First request | 2026-08-05 22:02 |
| Last request | 2026-08-07 19:14 |

## 2. By project

| Project | Requests | Input | Output | Cache created | Cache read |
|---|---:|---:|---:|---:|---:|
| `-Users-jyh-projects-claude-saltworks` | 9,736 | 55,190 | **6,580,115** | 34,285,691 | 2,839,327,501 |
| `-Users-jyh-projects-claude-salt` | 4,184 | 49,359 | **2,467,202** | 27,696,156 | 1,329,511,202 |
| **TOTAL** | **13,920** | **104,549** | **9,047,317** | **61,981,847** | **4,168,838,703** |

## 3. By model tier

| Tier | Requests | Input | Output | Cache created | Cache read |
|---|---:|---:|---:|---:|---:|
| Opus 5 | 12,731 | 90,336 | **7,980,209** | 52,323,865 | 3,557,885,436 |
| Fable 5 | 1,102 | 2,733 | **1,026,977** | 6,887,782 | 557,264,113 |
| Opus 4.8 | 81 | 153 | **40,121** | 2,703,363 | 53,509,845 |
...
| 2026-08-07 | 4,673 | 14,005 | **3,994,953** | 21,480,096 | 1,954,314,247 |
| **TOTAL** | **13,920** | **104,549** | **9,047,317** | **61,981,847** | **4,168,838,703** |
...
|---|---:|---:|---:|---:|---:|
| main loop | 7,255 | 17,543 | **8,426,779** | 30,456,706 | 3,616,031,806 |
| subagents / workflow agents | 6,665 | 87,006 | **620,538** | 31,525,141 | 552,806,897 |
| **TOTAL** | **13,920** | **104,549** | **9,047,317** | **61,981,847** | **4,168,838,703** |

_In this window the subagents made **48% of the requests** and **7% of the output tokens** (main loop: 52% / 93%). Design and orchestration sat in the main loops; the agents were many but individually cheap._
...
| Fact | Value |
|---|---:|
| Transcript files read | 2,433 |
| JSONL records scanned | 412,661 |
| Duplicate assistant records dropped (same `requestId`) | 127,283 |
| `<synthetic>` records dropped (API errors, zero usage) | 107 |
| Unparseable lines | 0 |

**The dedup rule.** Claude Code writes one assistant record per content block of a response, and **every one of those records repeats the whole `usage` block of the single API call**. Measured here: 127,283 records were duplicates of a request already counted. Summing records instead of requests would inflate every number in this file by roughly a factor of three. Usage was verified byte-identical within each `requestId` group before the rule was adopted.

**Subagents.** Workflow and Task agents write their own transcripts under `<session>/subagents/**/agent-*.jsonl`. They are included by default (`--no-subagents` to exclude). In THIS window they are 48% of requests and 7% of output tokens. **Request share and token share differ sharply and can point opposite ways** — quote whichever you mean, and never the word 'majority' unattached to a unit.
...
Generated 2026-08-07 19:14 America/Los_Angeles by `docs/ledger-tools/human_time.py`, per `docs/measurement-preregistration.md` §2.
...
|---|---:|---:|---:|
| **DIRECTING** | 4 | 7h 00m | 28.0% |
| **UNBLOCKING** | 1 | 6h 05m | 24.3% |
| WATCHING | 2 | 0h 02m | 0.2% |
| UNTAGGED | 10 | 11h 52m | 47.5% |
| **THE CLAIM** (D+R+U) | 5 | **13h 06m** | 52.4% |
| (all engaged time) | 17 | 25h 01m | 100% |

> **10 block(s), 11h 52m, are UNTAGGED.** They are counted in neither the claim nor WATCHING. Tag them in `EVIDENCE-human-time-tags.tsv` — one line per block id — and re-run. An untagged block is never silently folded into a category.
...
| 2026-08-07 | 8 | 11h 14m | 0h 00m | 224 |
...
| `20260807T1633` | salt | 08-07 16:33 | 19:14 | 2h 40m | 51 | **UNTAGGED** | legacy-fallback,typed (418 chars) |
...
- Touches read: 522 in window. Rejected as non-human across the whole record: 18,893.
...
Generated 2026-08-07 19:14 America/Los_Angeles by `docs/ledger-tools/landed.py`. Window: `2026-08-05 22:00` → `now`.
...
### `saltworks` — 430 commits

| Lane | Commits | Lines added | `.lean` added |
|---|---:|---:|---:|
| evidence | 149 | 17,153 | 0 |
| compiler (leg 2) | 124 | 17,290 | 11,402 |
| silicon (leg 3) | 95 | 182,947 | 2,809 |
| docs (shared) | 37 | 12,861 | 7,435 |
| maestro (hub) | 21 | 3,723 | 1,417 |
| maestro | 3 | 415 | 292 |
| other | 1 | 743 | 743 |
| **total** | **430** | **235,132** | **24,098** |
...
**500 commits across 2 repo(s) in the window.**


---

tile_drain: appended 2026-08-07T19:14:57-0700  tiles 202/512  pcbs 0/80
...
⛔ **NO SLOPE PRINTED — the series spans 6.7 h, need 36 h.** Readings close together measure the clock, not the shuttle.
