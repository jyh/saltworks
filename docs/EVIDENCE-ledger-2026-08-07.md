...
Generated 2026-08-07 06:27 America/Los_Angeles by `docs/ledger-tools/silence_windows.py` (saltworks, EVIDENCE seat).
...
A commit is made **by** a session, and a session writes records. So a commit landing where no personal-lane session wrote anything at all is **proof of a hole in the transcript record**, not evidence that nobody was directing. Checked against 358,115 liveness records at a **5-minute** tolerance.
...
| Separation | **634.1×** — **the threshold sits in an empty region** |

⛔ **1 of 198 commits landed in a stretch with NO transcript record.** Every silence figure below that contains one of these is a **lower bound on presence**: the human may have been directing and the evidence is missing, not absent. Do not publish a window containing these commits as unattended.
...
|---|---:|
| Commits | **198** |
| `.lean` lines inserted | **6,925** |
| All lines inserted | 49,877 |
| First commit | 2026-08-05 22:05 `4fa92be` |
| Last commit | 2026-08-07 06:00 `82155b2` |
| Human touches read, personal-lane fleet (whole transcript record) | **2,201** |
| — of which into this seat (`-Users-jyh-projects-claude-saltworks`) | 130 |
...
Inside the commit window itself, the fleet received **302** human touches: **127** into this seat and **175** into every other personal-lane seat combined.
...
| ≥ 8 h | 0 | 0.0% | 0 | 0 |
| ≥ 12 h | 0 | 0.0% | 0 | 0 |
| (all observed commits) | 198 | 100% | 6,925 | 49,877 |
...
| ≥ 8 h | 0 | 0.0% | 0 |
| ≥ 12 h | 0 | 0.0% | 0 |
| (observed by this seat) | 193 | 100% | 6,662 |
...
| 2026-08-07 | Fri | 1 | 0 | 0 | 17 | 01:45 | 06:27 |

## 7. The night column — reported so it is never quoted

Commits in 21:00–04:59 local: **19 of 198 (9.6%)**.
...
|---|---|---:|
| `typed` | **counted as human** | 1,952 |
| `slash-command` | **counted as human** | 151 |
| `legacy-fallback` | **counted as human** | 75 |
| `interrupt` | **counted as human** | 23 |
| `tool-result` | rejected | 13,467 |
| `task-notification` | rejected | 1,913 |
| `slash-command-echo` | rejected | 302 |
| `harness-injection` | rejected | 168 |
...
**Queue correction.** 846 messages were typed while the model was busy; the transcript writes them at dequeue time. Their `queue-operation: enqueue` timestamps were used instead. Largest correction applied: 464 s.

**Parse totals.** 105,370 records over 19 session files; 18,086 carried `type: user`; 0 lines failed to parse.
...
Generated 2026-08-07 06:28 America/Los_Angeles by `docs/ledger-tools/silence_windows.py` (saltworks, EVIDENCE seat).
...
A commit is made **by** a session, and a session writes records. So a commit landing where no personal-lane session wrote anything at all is **proof of a hole in the transcript record**, not evidence that nobody was directing. Checked against 358,116 liveness records at a **5-minute** tolerance.
...
✅ **Every one of the 67 commits in this window has a transcript record beside it.** The silences below are measured.
...
| All lines inserted | 10,924 |
| First commit | 2026-08-05 22:06 `db277c4` |
| Last commit | 2026-08-06 22:54 `f2efbab` |
| Human touches read, personal-lane fleet (whole transcript record) | **2,201** |
| — of which into this seat (`-Users-jyh-projects-claude-salt`) | 2,028 |
| Seats read for presence | 6 |
| Transcripts observe from | 2026-07-07 07:18 |
...
Inside the commit window itself, the fleet received **294** human touches: **172** into this seat and **122** into every other personal-lane seat combined.
...
| ≥ 8 h | 0 | 0.0% | 0 | 0 |
| ≥ 12 h | 0 | 0.0% | 0 | 0 |
| (all observed commits) | 67 | 100% | 4,489 | 10,924 |
...
| ≥ 8 h | 3 | 4.5% | 0 |
| ≥ 12 h | 0 | 0.0% | 0 |
| (observed by this seat) | 67 | 100% | 4,489 |
...
Commits in 21:00–04:59 local: **5 of 67 (7.5%)**.
...
**Parse totals.** 105,376 records over 19 session files; 18,087 carried `type: user`; 0 lines failed to parse.
...
A commit is made **by** a session, and a session writes records. So a commit landing where no personal-lane session wrote anything at all is **proof of a hole in the transcript record**, not evidence that nobody was directing. Checked against 358,128 liveness records at a **5-minute** tolerance.
...
Generated 2026-08-07 06:28 America/Los_Angeles by `docs/ledger-tools/token_meter.py` (saltworks, EVIDENCE seat), per `docs/measurement-preregistration.md` §1.
...
| Quantity | Tokens |
|---|---:|
| API requests (deduplicated) | 9,733 |
| Input | 91,451 |
| **Output** | **5,196,704** |
| Cache created | 42,333,301 |
| Cache read | 2,296,289,387 |
| First request | 2026-08-05 22:02 |
| Last request | 2026-08-07 06:28 |

## 2. By project

| Project | Requests | Input | Output | Cache created | Cache read |
|---|---:|---:|---:|---:|---:|
| `-Users-jyh-projects-claude-saltworks` | 7,262 | 50,565 | **3,674,835** | 24,601,124 | 1,593,797,232 |
| `-Users-jyh-projects-claude-salt` | 2,471 | 40,886 | **1,521,869** | 17,732,177 | 702,492,155 |
| **TOTAL** | **9,733** | **91,451** | **5,196,704** | **42,333,301** | **2,296,289,387** |

## 3. By model tier

| Tier | Requests | Input | Output | Cache created | Cache read |
|---|---:|---:|---:|---:|---:|
| Opus 5 | 9,025 | 78,147 | **4,509,562** | 37,625,763 | 1,997,571,385 |
| Fable 5 | 700 | 1,973 | **685,500** | 3,392,312 | 298,512,671 |
...
| 2026-08-07 | 486 | 907 | **144,340** | 1,831,550 | 81,764,931 |
| **TOTAL** | **9,733** | **91,451** | **5,196,704** | **42,333,301** | **2,296,289,387** |
...
|---|---:|---:|---:|---:|---:|
| main loop | 4,215 | 11,854 | **4,764,602** | 15,619,534 | 1,883,544,634 |
| subagents / workflow agents | 5,518 | 79,597 | **432,102** | 26,713,767 | 412,744,753 |
| **TOTAL** | **9,733** | **91,451** | **5,196,704** | **42,333,301** | **2,296,289,387** |
...
| **TOTAL** | **2,396** | **40,744** | **1,473,997** | **15,495,345** | **667,401,849** |

Attribution rule: each request is charged to the **next commit in `salt` at or after its timestamp**, if that commit lands within 4.0 h; otherwise it is unattributed. Unattributed in this window: 75 requests / 47,872 output tokens. Only requests from this repo's own seat (`-Users-jyh-projects-claude-salt`) are joined.
...
| Fact | Value |
|---|---:|
| Transcript files read | 2,404 |
| JSONL records scanned | 393,156 |
| Duplicate assistant records dropped (same `requestId`) | 122,912 |
| `<synthetic>` records dropped (API errors, zero usage) | 107 |
| Unparseable lines | 0 |

**The dedup rule.** Claude Code writes one assistant record per content block of a response, and **every one of those records repeats the whole `usage` block of the single API call**. Measured here: 122,912 records were duplicates of a request already counted. Summing records instead of requests would inflate every number in this file by roughly a factor of three. Usage was verified byte-identical within each `requestId` group before the rule was adopted.
...
Generated 2026-08-07 06:28 America/Los_Angeles by `docs/ledger-tools/human_time.py`, per `docs/measurement-preregistration.md` §2.
...
|---|---:|---:|---:|
| **DIRECTING** | 4 | 7h 00m | 48.9% |
| **UNBLOCKING** | 1 | 6h 05m | 42.4% |
| WATCHING | 2 | 0h 02m | 0.3% |
| UNTAGGED | 5 | 1h 12m | 8.4% |
| **THE CLAIM** (D+R+U) | 5 | **13h 06m** | 91.3% |
| (all engaged time) | 12 | 14h 21m | 100% |

> **5 block(s), 1h 12m, are UNTAGGED.** They are counted in neither the claim nor WATCHING. Tag them in `EVIDENCE-human-time-tags.tsv` — one line per block id — and re-run. An untagged block is never silently folded into a category.
...
| `20260807T0558` | salt | 08-07 05:58 | 06:27 | 0h 28m | 14 | **UNTAGGED** | legacy-fallback,typed (58 chars) |
...
- Touches read: 315 in window. Rejected as non-human across the whole record: 15,887.
...
Generated 2026-08-07 06:28 America/Los_Angeles by `docs/ledger-tools/landed.py`. Window: `2026-08-05 22:00` → `now`.
...
### `saltworks` — 198 commits

| Lane | Commits | Lines added | `.lean` added |
|---|---:|---:|---:|
| evidence | 105 | 13,278 | 0 |
| silicon (leg 3) | 42 | 26,412 | 1,773 |
| compiler (leg 2) | 36 | 5,961 | 3,473 |
| maestro (hub) | 10 | 3,684 | 1,387 |
| maestro | 3 | 415 | 292 |
| docs (shared) | 2 | 127 | 0 |
| **total** | **198** | **49,877** | **6,925** |
...
**265 commits across 2 repo(s) in the window.**
