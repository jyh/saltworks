...
⏳ **RETENTION — RULED BY COUNCIL 2026-08-28 (desk item 4, close 10:49): THE NDF ARTIFACTS ARE KEPT WHERE THEY ARE THROUGH CHIP RETURN; THE SUBMITTED SUBSET IS PERMANENT.** ⛔ **RETENTION IS PER-ARTIFACT, NOT PER-RUN — a run has no expiry at all, and I corrected the VALUE of this three times before questioning the UNIT.** Re-derived from the API 2026-08-28 10:5x, not carried from the bank: run `32284710003` — `tt_submission` 16 MB · `GDS_logs` 162 MB · `gds_render` · `precheck_reports` · `gatelevel_test_results` all live to **2026-11-17**, `github-pages` **ALREADY EXPIRED 2026-08-20**; run `31417665786` — the same five live to **2026-11-08**, `github-pages` **ALREADY EXPIRED 2026-08-11**. ⇒ *Two artifacts have already gone and the clocks that remain are per-file: after 2026-11-08 the earlier run's signoff evidence is unrecoverable from GitHub, chip return or not.*

📎 **AND THE ON-DISK HALF, WHICH IS A DIFFERENT OBJECT FROM THE CLOCKS ABOVE:** the ruling's own subject is the local archive, and its note now lives BESIDE THE MANIFEST where the ruling put it — `silicon-ndf-drv-0827.RETENTION.md` (a bare filename; it lives in the private archive, whose path is not written here) (search `RETIRE NO EARLIER THAN`), beside `silicon-ndf-drv-0827.SHA256SUMS`. ⛔ *Written 2026-08-28 17:1x because until then the routed note existed ONLY in this repo: a reader who opens that volume in 2027 sees 7.9 GB of unlabelled directories and no keep-until. The repo is the surface I could reach; the volume is the surface that gets read.*
