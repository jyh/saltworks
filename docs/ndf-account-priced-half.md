...
📎 **AND THE ON-DISK HALF, WHICH IS A DIFFERENT OBJECT FROM THE CLOCKS ABOVE:** the ruling's own subject is the local archive, and its note now lives BESIDE THE MANIFEST where the ruling put it — `/Volumes/Content HD/Saltworks/archives/silicon-ndf-drv-0827.RETENTION.md` (search `RETIRE NO EARLIER THAN`), beside `silicon-ndf-drv-0827.SHA256SUMS`. ⛔ *Written 2026-08-28 17:1x because until then the routed note existed ONLY in this repo: a reader who opens that volume in 2027 sees 7.9 GB of unlabelled directories and no keep-until. The repo is the surface I could reach; the volume is the surface that gets read.*
