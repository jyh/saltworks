...
> flagged its own README, and `prose_rot` opened a region that never closed and
> **SKIPPED THE WHOLE FILE — while printing `(A) FINDINGS 0`.*** ***An
> unswept file and a clean file print the same headline number; only the skip
> line distinguishes them, and the skip line is above the number.*** *Describe a
...
| `nightly.sh` | runs all of it, writes `docs/EVIDENCE-ledger-<date>.md`. ⛔⛔ **IT HAS PRODUCED NO LEDGER SINCE 2026-08-10 — measured 2026-09-03, 24 days — AND THERE ARE *TWO* INDEPENDENT REASONS, EITHER OF WHICH ALONE WOULD SUFFICE.** **(i) NOTHING INVOKES IT.** No crontab entry, no LaunchAgent, no watch under the seat's watch surface, no caller in the fleet tools; inside this repo every file naming `nightly.sh` is a doc or a ledger it wrote. **It is hand-run only.** *(I first wrote this row saying "every run produces nothing", which assumes runs occur — I inferred the frequency from the mechanism instead of measuring it, and corrected it on the bus within the hour. "It fails every night" and "it has not been run" produce the identical observable: an empty output directory.)* **(ii) AND IT WOULD FAIL IF RUN.** `set -e` (line 13) plus `python3 selftest.py` (line 43) is a deliberate hard gate, and it is right: its own comment says a partial ledger *"still LOOKED finished (header present, tables missing)"*. But the gate now fails permanently — `selftest.py` reported **1 failure of 178**, `✗ MANIFEST: bundle missing: docs/provenance/s2/s2-executor-transcript.jsonl`. ✅ **CLEARED 2026-09-11 — and by a DECLARATION, never by deleting the manifest row**, which would have made the gate green by destroying the provenance binding this directory exists to protect. The suite now reads **190 checks, 0 failures**, and the withheld declaration is itself gated: a row with no 64-hex sha256, no substantive reason, or naming a file that is actually PRESENT is a FAILURE, so the escape hatch cannot be used as a mute button. Five mutation controls driven, one of which caught the first version of the present-file guard iterating the wrong population., and that file is **not on `master` and unreachable from it**. ⛔⛔ **THIS SENTENCE PREVIOUSLY READ *"in no commit that ever existed, on no clone's disk, and is not gitignored"* AND ALL THREE CLAUSES WERE FALSE** — corrected 2026-09-11 at the object. It was added by `a5d2ef7` (2026-08-07, *"the artifact ships with its birth record"*, 4 files / 283 lines), that commit is present in two local clones, and the blob is intact at **558,073 bytes with a SHA-256 that matches `MANIFEST.tsv` exactly**. The commit is unreachable from `origin/master` and on no remote branch — which is a claim about REACHABILITY, and I had written it as a claim about EXISTENCE. ⇒ 🔑 ***"I CANNOT REACH IT" AND "IT DOES NOT EXIST" ARE THE SAME OBSERVATION FROM ONE CLONE, AND ONLY THE SECOND ONE FORECLOSES A REPAIR.*** The file is now formally **WITHHELD** (`docs/provenance/WITHHELD.tsv`) on lane grounds that were measured rather than reasoned: it carries two live chat-service session URLs, so restoring it would be refused by this repo's own Scrub CI. ⇒ ⭐ **A MANIFEST THAT BINDS AN ARTIFACT TO AN *UNTRACKED* FILE DECAYS TO A PERMANENT RED THE FIRST TIME ANYONE CLEANS A TREE — git cannot preserve the very thing the binding depends on.** ⛔ **AND THE DEATH IS INVISIBLE BY CONSTRUCTION: on failure the nightly emits NOTHING, so `EVIDENCE-ledger-latest.md` still reads `# CAMPAIGN LEDGER — 2026-08-10` and looks exactly like a ledger.** *A pipeline that publishes only on success leaves its last success standing as its own tombstone*, and nobody re-reads a file that has always been there. ⚠️ **Two further rots in that same manifest row, neither of which the missing file explains:** field 2 is a hardcoded `/Users/jyh/projects/claude/saltworks/…` fleet-root path (see `fleet_root_census.sh`), and field 4 points into `/Users/jyh/.claude/projects/…` — **the default config dir the fleet moved off when seats went per-account**, so the source transcript would not resolve even if the bundle file returned. ⛔ **NOT REPAIRED BY THE HEAD THAT FOUND THIS, DELIBERATELY.** Deleting the manifest row would destroy a provenance binding to make a gate green — the exact trade this directory exists to refuse — and resuming a dormant PUBLIC publication is not one seat's call. **Reported, routed, and recorded here rather than taken.** |
